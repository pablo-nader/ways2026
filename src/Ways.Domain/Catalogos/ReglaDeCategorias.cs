...
/// por escritura — <c>nivelDelPadre</c> (profundidad 1-indexada del padre elegido: 1 para
/// una raíz, 2 para un hijo de una raíz, etc.) y <c>alturaDelSubarbol</c> (altura del
/// subárbol que cuelga del nodo insertado o movido). No se guarda un nivel denormalizado:
/// reescribirlo en cada re-parent y el riesgo de que alguien lo desincronice escribiendo una
/// categoría por SQL no valen la pena para catálogos de unas pocas docenas de filas.
/// </summary>
public static class ReglaDeCategorias
{
    ...
    /// profundidad 1-indexada del padre elegido (1 = el padre es una raíz, 2 = el padre es
    /// hijo de una raíz…) — <b>no</b> la cantidad de sus ancestros (nótese que ambas
    /// coinciden en el caso "sin padre" = 0, pero no en ningún otro). <paramref
    /// name="alturaDelSubarbol"/>: 0 si el nodo no tiene hijos, o la altura del subárbol que
    /// cuelga de él.</summary>
