    ...
    InspectorDeUso inspector)
{
    ...
    public async Task<UsuarioListado> CrearAsync(CrearUsuario datos, CancellationToken ct = default)
    {
        ...
        // La entidad se construye ACÁ AFUERA, antes del lambda. Hoy la escritura corre sin
        // reintento (ef-retry-safe-writes, forma (b), abajo), así que el izado ya no es lo que
        // evita el doble alta — el no-reintento lo es. Se conserva igual porque `usuario` es el
        // dato que la auditoría de más abajo referencia por id: moverlo adentro no compraría
        // nada y obligaría a repetir el hasheo de la contraseña, que es caro por diseño.
        ...
        //
        // Sin reintento (ef-retry-safe-writes, forma (b)): `auditoria.Registrar` hace Add de una
        // instancia NUEVA en cada intento, así que un reintento duplicaría la fila de auditoría
        // del alta. El alta en sí no se duplicaría —`usuario` es siempre la misma instancia— pero
        // eso es una propiedad del izado, no una garantía de este lambda.
