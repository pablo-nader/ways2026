 ...
 * un acto deliberado del cajero (tipear 0, nunca `null`), nunca un default silencioso. */
export function conteoValido(valor: number | null): boolean {
 ...
 * `resumen !== null`. */
export function conteosCompletos(medios: LineaDeResumen[], valores: Record<number, number | null>): boolean {
  return medios.every((m) => conteoValido(valores[m.idMedioPago] ?? null))
}
...
export function aSolicitudDeCierre(
  ...
  const conteos: ConteoDeclarado[] = medios.map((m) => ({
    ...
    importeDeclarado: valores[m.idMedioPago] ?? 0,
  }))
