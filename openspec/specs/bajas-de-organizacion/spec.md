...
> Superseded by Reconciliación 11 (stage 20): no article-shaped row hangs off a punto de venta
> (`articulos` is tenant-wide); the tenant and the empresa block on the article, but the punto de
> venta blocks on its own smallest customer datum, one `stock` row instead — usage propagates UP
> the hierarchy, never down.
...
> Superseded by Reconciliación 4 (stage 20): the literal "fails the build on an unclassified type"
> is a tautology against a total classifier (carve-out → timestamped → untimestamped); delivered
> instead by N1 (mechanical-impossibility throws naming the type), N2, N3 (the checked-in sorted
> golden inventory — the actual trip-wire) and N5, not by a single bucket-membership assertion.
...
> Superseded by Reconciliación 4 (stage 20): two-bucket membership is unrepresentable by
> construction — the classifier is a total ordered cascade (carve-out, then timestamped, then
> untimestamped) — so the property is proved structurally by the classifier's shape plus
> `NingunCarveOutAportaRamaParaNingunaAncla`, not by a red test over a claimed double membership.
