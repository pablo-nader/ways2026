...
- [ ] 2.1 [P] Add `SugeridorDePrecio.Sugerir(costoNominal, costoLista,
...
- [ ] 2.2 Add `ServicioDeArticulos` (list/create/edit/soft-delete): create
...
- [ ] 2.3 Add `ServicioDeCodigosBarra` (or fold into `ServicioDeArticulos`
...
- [ ] 2.4 Add contracts: `AltaArticulo`/`EdicionArticulo`/`ArticuloListado`,
  `AltaCodigoBarra`, `SugerenciaDePrecio` (response DTO).

### 2C. API

- [ ] 2.5 Add `ArticulosEndpoints`: list/create/edit/soft-delete, barcode
  add/remove sub-routes, availability toggle, margin-suggestion read
  endpoint, `GestionDeCatalogo` policy (tenant admin only).

### 2D. Tests

- [ ] 2.6 [P] Unit: `SugeridorDePrecio` — grupo wins over proveedor, falls
...
- [ ] 2.7 [P] Unit (InMemory, where the write path allows it — same
...
- [ ] 2.8 [P] Integration: `codigo_interno` — concurrent omitted-value
...
- [ ] 2.9 [P] Integration: `codigos_barra` — cross-tenant same-barcode
...
- [ ] 2.10 [P] Integration: availability — default-true visible to a later
...
- [ ] 2.11 [P] Integration: FK smoke tests for each new `fk_articulos_*` and
  `fk_codigos_barra_articulo` (cross-tenant/nonexistent id → 23503/400).
  *(backstop map)*
- [ ] 2.12 Integration: admin create→soft-delete round trip; vendedor 403 on
...
- [ ] 2.13 Regression: Slice 1 suites unedited and green.
