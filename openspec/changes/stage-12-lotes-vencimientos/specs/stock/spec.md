...
integration test.
(Previously: silent on `id_lote` — the column did not exist until this
stage.)
...
The Purpose sentence MUST gain a clause noting that `movimientos_stock` now
carries an optional `id_lote` dimension and that `stock` itself is
unmodified in shape — the lot balance lives in a parallel `stock_lotes`
cache owned by the `lotes-y-vencimientos` capability.
