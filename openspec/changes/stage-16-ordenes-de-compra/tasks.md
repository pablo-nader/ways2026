...
- [ ] 1.1 Migration `OrdenesDeCompraEtapa16`: `CREATE TYPE estado_orden_compra AS ENUM
  ('borrador','enviada','recibida_parcial','cerrada','anulada')`. *(proposal.md:514-517, gate §A)*
- [ ] 1.2 Same migration: `CREATE TABLE ordenes_compra` — 16 columns exactly per §B (`numero
...
- [ ] 1.3 Same migration: 5 named FKs on `ordenes_compra` — `fk_..._tenant`, `fk_..._punto_venta`
...
- [ ] 1.4 Same migration: `ck_ordenes_compra_envio_completo` and `ck_ordenes_compra_cierre` exactly
  per §B's table. *(proposal.md:577-578)*
- [ ] 1.5 Same migration: 6 named indexes on `ordenes_compra` — `ix_..._tenant`,
  ...
  *(proposal.md:584-597)*
- [ ] 1.6 Same migration: `CREATE TABLE items_orden_compra` — 11 columns exactly per §C (no
  `cantidad_recibida`); `pk_items_orden_compra`. *(proposal.md:606-617)*
- [ ] 1.7 Same migration: 3 named FKs on `items_orden_compra` — `fk_..._tenant`,
...
- [ ] 1.8 Same migration: 3 named indexes on `items_orden_compra` — `ix_..._tenant`,
  `ix_..._orden_compra`, `ix_..._articulo`, `ux_items_orden_compra_orden` **UNIQUE**
  `(id_orden_compra, orden)` (4 total). *(proposal.md:641-644)*
- [ ] 1.9 Same migration: `ALTER TABLE comprobantes_compra ADD COLUMN id_orden_compra integer
...
- [ ] 1.10 Migration ordering verified in the generated file: `CREATE TYPE` → `CREATE TABLE
  ...
  *(proposal.md:721-724)*
- [ ] 1.11 Create `src/Ways.Domain/Compras/EstadoOrdenCompra.cs` — 5 values, member order = native
  type order. *(design.md:146)*
- [ ] 1.12 Create `src/Ways.Domain/Compras/ProyectorDeEstadoDeOrden.cs` — the pure five-arm rule,
  `PoliticaDeRoles` pattern, no database. *(design.md:148-156, decision 4)*
- [ ] 1.13 Create `src/Ways.Domain/Compras/OrdenCompra.cs` · `ItemOrdenCompra.cs` — `EntidadTenant`
  ⇒ `EntidadBase`, standard tenant filter + `EstamparTenant()`, like `ComprobanteCompra`.
  *(design.md:158-160, gate §B/§C)*
- [ ] 1.14 Create `OrdenCompraConfiguration.cs` · `ItemOrdenCompraConfiguration.cs` — shaped on
...
- [ ] 1.15 Modify `ComprobanteCompraConfiguration.cs` — `IdOrdenCompra` + FK 9 +
  `ix_comprobantes_compra_orden_compra` (named, never autogenerated). *(design.md:371, mutation
  target #9)*
- [ ] 1.16 Modify `WaysDbContext.cs` + `IWaysDbContext.cs` — two new `DbSet`s. *(design.md:372,
  374)*
- [ ] 1.17 Modify `WaysDbContextFactory.cs` **and** `DependencyInjection.cs` —
  `MapEnum<EstadoOrdenCompra>` in **both** builders, never also `HasPostgresEnum`.
  *(design.md:373, mutation target #8)*
- [ ] 1.18 Modify `docs/10-modelo-de-datos.md` — both tables (§5-adjacent), `comprobantes_compra.
...
- [ ] 1.19 Modify `src/Ways.Api/Seguridad/ManejadorDeErrores.cs` — 6 exact-name branches: (a)
  ...
  *(design.md:65-66, decisions 10-11; proposal.md:679, 688-689, gate §E)*
- [ ] 1.20 [P] Domain unit — `ProyectorDeEstadoDeOrden` full truth table: 2 estados × cierreManual
...
- [ ] 1.21 [P] Integration — RLS on `ways_app` (NOSUPERUSER NOBYPASSRLS): cross-tenant `SELECT`
  returns **0** rows on both new tables; `INSERT` with a foreign `id_tenant` refused `42501`.
  *(design.md:418, mutation targets #1-#2)*
- [ ] 1.22 [P] Integration — both CHECKs by raw insert, both directions, asserting SQLSTATE
...
- [ ] 1.23 [P] Integration — both item CHECKs by raw insert: `cantidad_pedida <= 0`,
  `costo_unitario_estimado < 0` — SQLSTATE `23514`. *(design.md:419, mutation target #5)*
- [ ] 1.24 [P] Integration — `ux_items_orden_compra_orden` by raw insert (server-assigned `orden`,
  race-test exemption documented, same family as `ux_items_comprobante_compra_orden`).
  *(proposal.md:680)*
- [ ] 1.25 [P] Integration — **the ordering trap, raw out-of-band insert**: a duplicate `numero`
  ...
  gate test (c))*
- [ ] 1.26 [P] Integration — `pg_indexes` shows **exactly 12** new indexes and no unnamed
  EF-generated FK support index; `has-pending-model-changes` clean. *(design.md:419, mutation
  target #9)*
- [ ] 1.27 [P] `db-error-backstops` exemption tests — FK 1/FK 6 (`_tenant`), FK 4/FK 5
