...
Stage 5 implements `motivo` values `venta`, `anulacion`, `ajuste` only —
`compra`, `transferencia`, `inventario` are reserved enum values with no
write path yet.
...
`stock` MUST be operativa-scoped (`id_tenant` + `id_punto_venta`, doc 09) with
`PRIMARY KEY (id_articulo, id_punto_venta)` and `cantidad` as a cache
maintained in the same transaction as the originating `movimientos_stock`
insert. `movimientos_stock` rows are immutable — no update endpoint MUST ever
exist for a movement row.
