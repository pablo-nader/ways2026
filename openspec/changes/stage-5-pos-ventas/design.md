...
| 11 | **`id_empleado` = `IContextoDeUsuario.UsuarioId`**, composite FK to `usuarios (id_usuario, id_tenant)` — **superseded at implementation (judgment-day R1, Slice 3): simple FK to `usuarios.id_usuario`**; the AK on `Usuario` would force `IdTenant NOT NULL`, corrupting the platform-staff NULL sentinel (verified by scaffold). `id_empleado` is always server-derived from `IContextoDeUsuario.UsuarioId`, never client input — Slice 4 MUST preserve that invariant | Create an `empleados` table now | No `empleados` table exists in stages 1–4 and doc 10 does not define one; the operator of a sale *is* the authenticated user. Recorded as an Open Question for the day payroll/employee data is modelled |
