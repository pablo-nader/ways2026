...
- [x] 1.15 Extend `InicializadorDeBaseDeDatos` with tenant 1 / empresa 1 / 2 locales seed, platform mode. *(spec: tenant-organization / Organization Hierarchy Tables, Scenario: Seed data present)* — code complete and the migration now exists to run it against; not yet exercised end-to-end because the Docker daemon is unreachable in this environment (see 1.17). Seed literal names ("Ways" / "Local 1" / "Local 2") stay as placeholders per the gate approval (decision 3).
- [x] 1.16 Scaffold `tests/Ways.IntegrationTests`: `WebApplicationFactory` + `Testcontainers.PostgreSql`, two DB roles (migration owner + `ways_app` `NOSUPERUSER NOBYPASSRLS`). *(ADR-17)* — fixture now runs the migration as `ways_owner` and provisions `ways_app` (`LOGIN NOSUPERUSER NOBYPASSRLS` + data-only `GRANT`s) directly against the container in `InitializeAsync`; the API host (`ConfigureWebHost`) is wired to connect as `ways_app`, never as the owner.
- [x] 1.17 Integration tests — isolation core: EF filter blocks cross-tenant read; RLS blocks raw-SQL/`IgnoreQueryFilters` read; `WITH CHECK` rejects cross-tenant insert; fail-closed on unset GUC; no GUC leakage across pooled connections; policy-coverage query (ADR-15) returns zero rows (now covering `tenants` too, per gate decision 2); `ways_app` has neither `rolsuper` nor `rolbypassrls`. *(spec: tenant-organization / Tenant Isolation Enforcement)* — **Implemented in full** (`AislamientoDeTenantTests.cs`, `[Fact]`, no longer `Skip`). Compiles clean. **Not runtime-verified**: the Docker daemon is unreachable in this environment (`docker version` connects the client but not the daemon — confirmed both before and after this batch); `dotnet test` on this project fails all 7 cases at the fixture constructor with `Testcontainers`' own `ArgumentException` ("Docker is either not running or misconfigured"), not inside any assertion. This is the expected, honest state per the coordinator's instruction: implemented and gated only by the daemon, not faked green. First thing to run once Docker is available: `dotnet test tests/Ways.IntegrationTests`.
- [x] 1.18 Regression: confirm existing `Ways.Domain.Tests`/`Ways.Application.Tests` are unedited and green. — Unedited; full suite: `Ways.Domain.Tests` 30/30, `Ways.Application.Tests` 14/14, `Ways.IntegrationTests` 0/7 (all 7 fail at the fixture constructor — Docker daemon unreachable, see 1.17). 0 build warnings.
