...
| Evidence | Value |
|---|---|
| Focused test command and exact result | `npx vitest run ConsultaPrecios` (`src/Ways.Web`) — **14/14 tests green** in the new colocated `ConsultaPrecios.test.tsx` (4 display-branch tests, 3 idle-reset/generation tests under `vi.useFakeTimers`, 3 pure-mapper `aResultadoDeConsulta` tests, 2 zero-writes/call-log tests, 2 no-session-bypass tests) |
